...
class PlayerIdPopulatorAgent(BaseAgent):
    model_config = {"arbitrary_types_allowed": True, "extra": "ignore"} # extra: ignore is safer
...
async def _collect_tools_stack(
    ...
    try:
        ...
        expected_tool_keys = [
            ...
            "video_clip_generator_mcp", "audio_processing_mcp"
        ]
...
3.  Output (CRITICAL): Tool responds `{"name":"...", "response":{"result":{"content":[{"text":"[{\"word\": ...}]"}]}}}`. Extract inner `"text"` value.
    ...
    final_recap_output_agent = LlmAgent(
        name="FinalRecapOutputter",
        model=GEMINI_FLASH_MODEL_ID, # Simple passthrough
        instruction="Output the text from session state key 'current_recap'.",
        # No tools needed, no output_key needed as its direct response is the final one from this sequence.
    )

    game_recap_assistant_v2 = SequentialAgent(
        name="game_recap_assistant_v2", # New name for the parallel version
        sub_agents=[
            script_generation_pipeline_instance,    # Step 1: Generate script
            parallel_asset_pipelines_instance,      # Step 2: Generate assets in parallel
            asset_validator_instance,               # Step 3: Validate and retry
            final_recap_output_agent                # Step 4: Provide the final recap text as output
        ],
        description="Orchestrates game recap script generation, concurrent multi-modal asset creation, validation, and final output."
    )
