...
# Removed: import logging.handlers # FileHandler is in 'logging' directly
...
################################################################################
# --- IterativeAssetSearchAgent Definition ---
################################################################################

class IterativeAssetSearchAgent(BaseAgent):
    """
    A custom agent that iterates through a list of asset search queries,
    invokes the appropriate searcher LLM for each, processes the results,
    and accumulates a list of found assets.
    """
    model_config = {"arbitrary_types_allowed": True}
    logo_searcher_llm: LlmAgent
    headshot_retriever_llm: LlmAgent
    # No entity_extractor or query_generator here; those are prerequisites.
    ...
    def __init__(self, name: str,
        ...
        super().__init__(
            ...
            # These are the LlmAgents this agent will call, not orchestrate as sub-agents in a fixed workflow
            sub_agents=[logo_searcher_llm, headshot_retriever_llm]
        )

    # Helper methods can be part of this class or utility functions
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        logger.info(f"[{self.name}] Starting iterative asset search.")
        found_assets_list = [] # Initialize local accumulator

        # Get prerequisites from session state
        ...
        player_lookup_dict = {}
        try:
            player_lookup_dict = json.loads(_clean_json_string_from_llm(player_lookup_dict_json_str, default_if_empty="{}"))
            if not isinstance(player_lookup_dict, dict):
                player_lookup_dict = {}
        except json.JSONDecodeError:
            logger.error(f"[{self.name}] Failed to parse player_lookup_dict_json: '{player_lookup_dict_json_str}'.")
        player_name_to_id_map_lower = {str(name).lower(): str(id_) for id_, name in player_lookup_dict.items() if name and id_}

        logger.info(f"[{self.name}] Iterating over {len(search_queries)} queries. Player ID map size: {len(player_name_to_id_map_lower)}")

        if not search_queries:
            logger.info(f"[{self.name}] No search queries to process.")
            ctx.session.state["current_static_assets_list"] = []
            yield Event(author=self.name, content=types.Content(role="model", parts=[types.Part(text="No static asset queries to process.")]))
            return
        ...
        for original_query_string in search_queries:
            ...
            if team_name_for_logo:
                ...
                ctx.session.state["team_name_for_logo_search"] = team_name_for_logo # Set state for LlmAgent
                async for event in self.logo_searcher_llm.run_async(ctx): yield event

                logo_tool_output_str = ctx.session.state.get("logo_search_result_json")
                cleaned_logo_tool_output_str = _clean_json_string_from_llm(logo_tool_output_str, default_if_empty='{}')
                try:
                    tool_response_obj = json.loads(cleaned_logo_tool_output_str)
                    logo_results_list = []
                    actual_data_str = None
                    if isinstance(tool_response_obj, dict):
                        response_key = "search_similar_images_by_text_response"
                        if response_key in tool_response_obj and \
                           isinstance(tool_response_obj[response_key].get("result"), dict) and \
                           isinstance(tool_response_obj[response_key]["result"].get("content"), list) and \
                           len(tool_response_obj[response_key]["result"]["content"]) > 0 and \
                           isinstance(tool_response_obj[response_key]["result"]["content"][0].get("text"), str):
                            actual_data_str = tool_response_obj[response_key]["result"]["content"][0]["text"]
                            logo_results_list = json.loads(actual_data_str)
                        elif tool_response_obj.get("error"):
                            logger.error(f"[{self.name}] Logo search tool reported error: {tool_response_obj.get('error')}")
                        elif isinstance(cleaned_logo_tool_output_str, str) and cleaned_logo_tool_output_str.strip().startswith("["):
                             logo_results_list = json.loads(cleaned_logo_tool_output_str)


                    if logo_results_list and isinstance(logo_results_list, list) and logo_results_list[0].get("image_uri"):
                        asset_info = logo_results_list[0]
                        asset_info["search_term_origin"] = original_query_string
                        asset_info.setdefault("type", "logo")
                        found_assets_list.append(asset_info)
                        logger.info(f"[{self.name}] Successfully processed logo: {asset_info.get('image_uri')}")
                    else:
                        logger.warning(f"[{self.name}] No valid logo URI for '{team_name_for_logo}'. Parsed list: {logo_results_list}")
                except Exception as e:
                    logger.error(f"[{self.name}] Error processing logo result for '{team_name_for_logo}': {e}. String: '{cleaned_logo_tool_output_str}'")

            elif player_name_for_headshot:
                logger.info(f"[{self.name}] Identified as HEADSHOT query for player: '{player_name_for_headshot}'")
                player_name_lower = player_name_for_headshot.lower()
                player_id = player_name_to_id_map_lower.get(player_name_lower)
                original_player_name_for_log = player_lookup_dict.get(player_id, player_name_for_headshot)

                if player_id:
                    logger.info(f"[{self.name}] Found Player ID '{player_id}' for '{original_player_name_for_log}'.")
                    ...
                    headshot_tool_output_str = ctx.session.state.get("headshot_uri_result_json")
                    ...
                    try:
                        ...
                        if isinstance(headshot_result_dict, dict):
                            if headshot_result_dict.get("error"):
                                logger.error(f"[{self.name}] Headshot tool error for '{original_player_name_for_log}': {headshot_result_dict['error']}")
                            elif headshot_result_dict.get("image_uri"):
                                asset_info = headshot_result_dict
                                asset_info["search_term_origin"] = original_query_string
                                asset_info.setdefault("type", "headshot")
                                asset_info.setdefault("entity_name", original_player_name_for_log)
                                asset_info.setdefault("entity_id", player_id)
                                found_assets_list.append(asset_info)
                                logger.info(f"[{self.name}] Successfully retrieved headshot for '{original_player_name_for_log}': {asset_info.get('image_uri')}")
                            else:
                                logger.warning(f"[{self.name}] No headshot URI for '{original_player_name_for_log}'. Result: {cleaned_headshot_tool_output_str}")
                        else:
                             logger.error(f"[{self.name}] Headshot tool output not a dict for '{original_player_name_for_log}': {cleaned_headshot_tool_output_str}")
                    except Exception as e:
                        logger.error(f"[{self.name}] Error processing headshot result for '{original_player_name_for_log}': {e}. String: '{cleaned_headshot_tool_output_str}'")
                else:
                    logger.warning(f"[{self.name}] Player ID NOT FOUND for headshot query: '{player_name_for_headshot}' (searched as '{player_name_lower}')")
            else:
                logger.warning(f"[{self.name}] Unrecognized query format, skipping: '{original_query_string}'")

        ctx.session.state["current_static_assets_list"] = found_assets_list # Store final list
        logger.info(f"[{self.name}] Iterative asset search finished. Found {len(found_assets_list)} assets.")
        if found_assets_list:
            logger.info(f"[{self.name}] Details of found static assets: {json.dumps(found_assets_list, indent=2)}")
        yield Event(author=self.name, content=types.Content(role="model", parts=[types.Part(text=f"Iterative asset search complete. Found {len(found_assets_list)} assets.")]))




################################################################################
# --- PHASE 1: REFACTORED StaticAssetPipelineAgent (Python-driven retrieval loop) ---
################################################################################
# --- StaticAssetPipelineAgent class ---
class StaticAssetPipelineAgent(BaseAgent): # Now a custom BaseAgent orchestrator
    model_config = {"arbitrary_types_allowed": True}
    entity_extractor: LlmAgent
    static_asset_query_generator: LlmAgent
    iterative_asset_search_agent: IterativeAssetSearchAgent # NEW: The agent that loops

    def __init__(self, name: str,
                 entity_extractor: LlmAgent,
                 static_asset_query_generator: LlmAgent,
                 iterative_asset_search_agent: IterativeAssetSearchAgent): # Pass the new agent
        super().__init__(
            name=name,
            entity_extractor=entity_extractor,
            static_asset_query_generator=static_asset_query_generator,
            iterative_asset_search_agent=iterative_asset_search_agent, # Store it
            sub_agents=[ # These are the agents this orchestrator will call IN SEQUENCE
                entity_extractor,
                static_asset_query_generator,
                iterative_asset_search_agent
            ]
        )

    @override
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        logger.info(f"[{self.name}] Starting static asset pipeline (now with IterativeAssetSearchAgent).")
        ctx.session.state["current_static_assets_list"] = [] # Initialize final output key
        ...
        async for event in self.entity_extractor.run_async(ctx): # Use stored instance
            yield event
        # Output "extracted_entities_json" is set in session state by entity_extractor

        # Step 2: Generate Search Queries
        # PlayerIdExtractorAgent should have already run and set "player_lookup_dict_json"
        # if it's part of the broader GameRecapAgent flow before this pipeline.
        # StaticAssetQueryGenerator relies on "extracted_entities_json".
        logger.info(f"[{self.name}] Running StaticAssetQueryGenerator...")
        async for event in self.static_asset_query_generator.run_async(ctx): # Use stored instance
            yield event
        # Output "static_asset_search_queries_json" is set in session state

        # Step 3: Perform Iterative Asset Search using the new agent
        # This agent will use "static_asset_search_queries_json" and "player_lookup_dict_json"
        # from session state and will place "current_static_assets_list" in state.
        logger.info(f"[{self.name}] Running IterativeAssetSearchAgent...")
        async for event in self.iterative_asset_search_agent.run_async(ctx): # Use stored instance
            yield event

        final_found_assets_count = len(ctx.session.state.get("current_static_assets_list", []))
        logger.info(f"[{self.name}] Static asset pipeline finished. IterativeAssetSearchAgent found {final_found_assets_count} assets.")
        yield Event(author=self.name, content=types.Content(role="model", parts=[types.Part(text=f"Static asset pipeline complete. Found {final_found_assets_count} assets.")]))
...
# --- PlayerIdExtractorAgent Definition ---
class PlayerIdExtractorAgent(LlmAgent):
    """
    An agent dedicated to extracting player IDs and names from game data
    and storing them as a JSON lookup in session state.
    """
    model_config = {"arbitrary_types_allowed": True} # Allow mlb_tools attribute

    def __init__(self, name: str, model_id: str, mlb_tools: list):
        super().__init__(
            name=name,
            model=model_id,
            instruction="""
You are a specialized data extraction agent. Your SOLE task is to create a player ID to player name lookup dictionary for an MLB game.

Session State Expectations:
- `game_pk`: The unique ID for the game (e.g., "717527"). This MUST be present and a non-empty string.

Your Process:
1.  Retrieve `game_pk` from session state. If `game_pk` is missing, empty, or not a string, output an error JSON: `{"error": "Valid game_pk is missing from session state"}` and STOP.
2.  Call the `mlb_stats.get_game_boxscore_and_details` tool using the `game_pk` from session state.
3.  If the tool call fails or returns no substantive player data within the boxscore (e.g., under `boxscore.teams.home.players` or `boxscore.teams.away.players` which usually are dictionaries of player_id: player_object), output an error JSON: `{"error": "Failed to fetch or process boxscore data, or no player data found in boxscore"}` and STOP.
4.  **Extract Player Data:**
    *   From the data returned by the tool (typically a complex JSON object representing the boxscore), navigate to the sections containing player rosters for both home and away teams (e.g., paths like `boxscore.teams.home.players` and `boxscore.teams.away.players`). These sections usually contain dictionaries where keys are player IDs and values are objects containing player details including their full names.
    *   Iterate through all players found for both teams.
    *   For each player, extract their official player ID (which is the key in the player dictionary, e.g., "123456") and their full name (e.g., from a field like `fullName` or `person.fullName` within the player object).
5.  **Create Lookup Dictionary:**
    *   Construct a Python dictionary where keys are player IDs (as strings) and values are their full names (e.g., `{{"12345": "Player Name One", "67890": "Player Name Two"}}`).
6.  **Output as JSON String:**
    *   Convert this dictionary into a JSON string.
    *   If no players can be extracted (e.g., empty rosters in boxscore data), output an empty JSON object string: `'{{}}'`.
    *   If a critical error occurred as defined above, output the specific error JSON.

This final JSON string is your ONLY output. Do not add any other text, conversation, or explanation.
            """,
            tools=mlb_tools,  # Ensure it gets the necessary mlb_stats tools
            output_key="player_lookup_dict_json" # Stores the JSON string directly
        )



################################################################################
# --- GameRecapAgent Definition (Modular Audio, Full Pipeline) ---
################################################################################
class GameRecapAgent(BaseAgent):
    model_config = {"arbitrary_types_allowed": True}
    initial_recap_generator: LlmAgent
    player_id_extractor: PlayerIdExtractorAgent 
    ...
    visual_asset_workflow_orchestrator: VisualAssetWorkflowAgent
    text_to_speech_agent: TextToSpeechAgent             # Updated
    speech_to_timestamps_agent: SpeechToTimestampsAgent # Updated
    ...
    def __init__(
        ...
        text_to_speech_agent: TextToSpeechAgent,             # Updated
        speech_to_timestamps_agent: SpeechToTimestampsAgent, # Updated
        # AssetValidator is created internally
    ):
        refinement_loop = LoopAgent(
        ...
        asset_validator_instance = AssetValidationAndRetryAgent(
            ...
            text_to_speech_agent=text_to_speech_agent,                         # Pass the TTS agent instance
            speech_to_timestamps_agent=speech_to_timestamps_agent,             # Pass the STT agent instance
            max_retries_per_pipeline=1 
        )
        ...
        sub_agents_list = [
            ...
            visual_asset_workflow_orchestrator,
            text_to_speech_agent,             # Add TTS agent
            speech_to_timestamps_agent,       # Add STT agent
            asset_validator_instance,
        ]
        super().__init__(
            name=name,
            initial_recap_generator=initial_recap_generator,
            player_id_extractor=player_id_extractor,
            ...
            text_to_speech_agent=text_to_speech_agent,                 # Pydantic field
            speech_to_timestamps_agent=speech_to_timestamps_agent,     # Pydantic field
    ...
    @override
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        logger.info(f"[{self.name}] Starting FULL game recap and asset workflow (modular audio).")

        # --- 0. Pre-requisites (Same as before) ---
        # ... (user_query and player_lookup_dict_json setup) ...
        ...
        # --- 1. Dialogue Generation & Refinement (Same as before) ---
        ...
        # ... (check for early exit, refinement_loop, post_processing_sequence) ...
        ...
        if not final_dialogue_recap:
            logger.error(f"[{self.name}] Dialogue recap is empty after refinement. Aborting full pipeline.")
        ...
        # --- 1b. Player ID Extraction ---
        logger.info(f"[{self.name}] Running PlayerIdExtractorAgent")
        async for event in self.player_id_extractor.run_async(ctx): # Use stored agent instance
            yield event
        player_lookup_json = ctx.session.state.get("player_lookup_dict_json", "{}")
        logger.info(f"[{self.name}] PlayerIdExtractorAgent finished. player_lookup_dict_json: {player_lookup_json[:100]}...")
        try:
            parsed_lookup = json.loads(player_lookup_json)
            if isinstance(parsed_lookup, dict) and parsed_lookup.get("error"):
                logger.error(f"[{self.name}] PlayerIdExtractorAgent reported an error: {parsed_lookup.get('error')}. Static asset retrieval might fail.")
                # Decide if to stop the whole pipeline or proceed with missing player IDs
        except json.JSONDecodeError:
            logger.error(f"[{self.name}] Failed to parse player_lookup_dict_json from PlayerIdExtractorAgent. Content: {player_lookup_json}")
        ...
        async for event in self.visual_asset_workflow_orchestrator.run_async(ctx):
            yield event
        logger.info(f"[{self.name}] VisualAssetWorkflow Orchestrator finished.")

        # --- 3. Audio Processing Workflow (Modular) ---
        if final_dialogue_recap:
            logger.info(f"[{self.name}] Running TextToSpeechAgent...")
            async for event in self.text_to_speech_agent.run_async(ctx):
                yield event
            generated_audio_uri = ctx.session.state.get("generated_dialogue_audio_uri")
            logger.info(f"[{self.name}] TextToSpeechAgent finished. Audio URI: {generated_audio_uri}")

            if generated_audio_uri:
                logger.info(f"[{self.name}] Running SpeechToTimestampsAgent...")
                async for event in self.speech_to_timestamps_agent.run_async(ctx):
                    yield event
                logger.info(f"[{self.name}] SpeechToTimestampsAgent finished. Timestamps: {len(ctx.session.state.get('word_timestamps_list',[]))}")
            else:
                logger.warning(f"[{self.name}] Skipping SpeechToTimestampsAgent as no audio URI was generated by TTS.")
        else:
            logger.warning(f"[{self.name}] Skipping all Audio Processing as final dialogue recap is empty.")

        # --- 4. Asset Validation and Retry Step ---
        if self.asset_validator and self.asset_validator.max_retries_per_pipeline > 0:
        ...
        # --- Final Logging & Output (Same as before) ---
        all_image_assets_final = ctx.session.state.get("all_image_assets_list", [])
        all_video_assets_final = ctx.session.state.get("all_video_assets_list", [])
        audio_uri_final = ctx.session.state.get("generated_dialogue_audio_uri")
        timestamps_final_count = len(ctx.session.state.get("word_timestamps_list", []))
        logger.info(f"[{self.name}] Post-Validation - Total Images: {len(all_image_assets_final)}, Videos: {len(all_video_assets_final)}, Audio: {'Yes' if audio_uri_final else 'No'}, Timestamps: {timestamps_final_count}")
        
        yield Event(author=self.name, content=types.Content(role="model", parts=[types.Part(text=final_dialogue_recap)]))
        logger.info(f"[{self.name}] === GameRecapAgent FULL processing complete (modular audio). ===")
...
4.  After processing all queries, convert `found_assets_list` into a JSON string. This is your final output.
    ...
    # --- NEW Simpler LlmAgents for individual tool calls within StaticAssetPipelineAgent ---
    logo_searcher_llm_agent = LlmAgent( # Renamed from logo_searcher_llm for consistency
        name="LogoSearcherLlm",
        model=GEMINI_FLASH_MODEL_ID, 
        instruction="""
You are a specialized agent. Your ONLY task is to execute the `image_embedding_mcp.search_similar_images_by_text` function tool.
...
- `top_k`: 1
- `filter_image_type`: "logo"
Execute this tool call. Do not add any other text, conversation, or explanation before or after initiating the tool call. Your response should solely be the outcome of this tool usage.
...
""",
    ...
    headshot_retriever_llm_agent = LlmAgent( # Renamed from headshot_retriever_llm for consistency
...
""",
    ...
    model=GEMINI_FLASH_MODEL_ID,
    instruction="""
You are a specialized agent. Your ONLY task is to execute the `static_retriever_mcp.get_headshot_uri_if_exists` function tool.
You will be given `player_id_for_headshot_search` and `player_name_for_headshot_log` in session state.
Use these exact parameters for the tool call:
- `player_id_str`: {session.state.player_id_for_headshot_search}
- `player_name_for_log`: {session.state.player_name_for_headshot_log}
...
3.  **Output Generation (CRITICAL - Precise Extraction):**
    ...
    # --- Instantiate NEW PlayerIdExtractorAgent ---
    mlb_tools_for_id_extractor = loaded_mcp_tools.get("mlb", []) # Or specifically mlb_stats if separated
    player_id_extractor_agent_instance = PlayerIdExtractorAgent(
        name="PlayerIdExtractor",
        model_id=GEMINI_FLASH_MODEL_ID, # Can be flash if instructions are clear enough
        mlb_tools=mlb_tools_for_id_extractor
    )

    # --- Instantiate NEW IterativeAssetSearchAgent ---
    iterative_asset_search_agent_instance = IterativeAssetSearchAgent(
        name="IterativeAssetSearcher",
    ...
    static_asset_pipeline_agent_instance = StaticAssetPipelineAgent(
        ...
        iterative_asset_search_agent=iterative_asset_search_agent_instance 
    )
    ...
    game_recap_assistant = GameRecapAgent(
        ...
        player_id_extractor=player_id_extractor_agent_instance, # 
